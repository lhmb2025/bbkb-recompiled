...
GRADLE = os.path.join(support.REPO_ROOT, "app", "build.gradle")  # the real one, shape only
...
class GradleVersionTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.version = appbuild.read_gradle_version(support.write_fixture_gradle(self.tmp))

    def tearDown(self):
        shutil.rmtree(self.tmp, ignore_errors=True)

    def test_the_fixture_gradle_file_is_read(self):
    ...
    def test_the_real_gradle_file_has_the_shape_the_parser_expects(self):
        real = appbuild.read_gradle_version(GRADLE)
        self.assertGreater(real.version_code, 1429)
        self.assertRegex(real.version_name, r"^\d+\.\d+\.\d+")
        self.assertEqual(23, real.min_sdk)
        self.assertEqual("-debug", real.suffixes["debug"])
