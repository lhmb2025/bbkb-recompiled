...
FIXTURE_GRADLE_TEXT = """android {
    defaultConfig {
        minSdk = 23
        versionCode = 1429  //  Most recent build is 1429
...
def write_fixture_gradle(directory):
    """A build.gradle shaped like the app's, pinned to the values these tests assert on,
    so the suite does not break at every version bump of the real file."""
    path = os.path.join(directory, "build.gradle")
    with open(path, "w") as handle:
        handle.write(FIXTURE_GRADLE_TEXT)
    return path
